	...
	public void compresion(BufferedImage image, int[] alto, int[] ancho, Nodo actual) throws EInfo {
		...
		if(alto[0] == alto[1] && ancho[0] == ancho[1]) {
			...
			if(pixel == match.getRGB()) {
				...
				//Puede que se necesite restar 1 al altoX despues de salir de ambos ciclos
				...
				if(altoX >= alto[1] && anchoY >= ancho[1]) {
					...
					//Arriba izquierda
					//Puede que se necesite sumr 1 a algunos rangos de los sectores para evitar leer el mismo pixel
					//Nullpointer exception en la llamada del hijo del nodo actual
